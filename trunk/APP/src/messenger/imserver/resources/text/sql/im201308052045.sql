...
INSERT INTO `LoginHistories` VALUES (3,'hehui','200.200.200.17',100,NULL,NULL,'2013-07-01 17:12:45','2013-07-02 01:13:49',NULL,NULL,NULL);
...
INSERT INTO `PersonalContactGroups` VALUES (1001,'hehui','CG1',1,1),(1002,'hehui','TG1',1,1);
...
CREATE TABLE `PersonalRelationship` (
  ...
  CONSTRAINT `FK-UF_PCGID-PCG_CGID` FOREIGN KEY (`PersonalContactGroup`) REFERENCES `PersonalContactGroups` (`GroupID`),
...
INSERT INTO `UsersDetailedInfo` VALUES (1,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL);
...
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=utf8 COMMENT='基本信息表';
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `UsersSummaryInfo` VALUES (1,'hehui','s0jwcxN/IhFAz7GaJEk29xPToSw=',NULL,NULL,0,0,NULL,0,0,0,1,1,'0','0',3,NULL,1),(2,'admin','s0jwcxN/IhFAz7GaJEk29xPToSw=',NULL,NULL,0,0,NULL,0,0,0,1,1,'0','0',3,NULL,1),(9,'test','s0jwcxN/IhFAz7GaJEk29xPToSw=',NULL,NULL,0,0,NULL,0,0,0,1,1,'0','0',3,NULL,1),(10,'test2','s0jwcxN/IhFAz7GaJEk29xPToSw=',NULL,NULL,0,0,NULL,0,0,0,1,1,'0','0',3,NULL,1);
...
--
/*!50003 DROP PROCEDURE IF EXISTS `sp_AddOrDeleteBlacklistedContactForUser` */;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`hehui`@`200.200.200.17` PROCEDURE `sp_AddOrDeleteBlacklistedContactForUser`(in p_userID varchar(16), in p_contactID varchar(16), in p_addToBlacklist tinyint )
BEGIN
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`hehui`@`%` PROCEDURE `sp_ContactGroup_UpdateName`(in p_userID varchar(16), in p_groupID int(10) unsigned , in p_newGroupName varchar(32), out p_groupInfoVersion int(10) unsigned )
BEGIN

    set p_groupInfoVersion = 0; 
	...
	select GroupInfoVersion into p_groupInfoVersion from PersonalContactGroups where Creator = p_userID and GroupID = p_groupID ;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`hehui`@`%` PROCEDURE `sp_CreateNewUser`(in u_id varchar(16), in u_password varchar(32), out u_sysid int(10) unsigned )
BEGIN
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sp_DeleteContactForUser` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8 */ ;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`hehui`@`200.200.200.17` PROCEDURE `sp_DeleteContactForUser`(in p_userID varchar(16), in p_contactID varchar(16), in p_deleteMeFromOpposition tinyint, in p_addToBlacklist tinyint )
BEGIN
...
if p_addToBlacklist <> 0 then
	call sp_AddOrDeleteBlacklistedContactForUser(p_userID, p_contactID, 1);
	#insert into PersonalBlacklist values(p_userID, p_contactID, NULL) ;
	#update UsersSummaryInfo set BlacklistInfoVersion = BlacklistInfoVersion+1 where UserID = p_userID;
end if;
...
where pr.UserID = p_userID and usi.UserID = pr.ContactID and pr.PersonalContactGroup = pg.GroupID 
...
select  GROUP_CONCAT( (select CONCAT_WS(',', usi.UserID, usi.PersonalSummaryInfoVersion, usi.PersonalDetailInfoVersion, pr.PersonalContactGroup)) SEPARATOR ';' ) as Contacts
from  UsersSummaryInfo usi, PersonalRelationship pr 
where pr.UserID = p_userID and usi.UserID = pr.ContactID 
;#group by pr.PersonalContactGroup;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`hehui`@`200.200.200.17` PROCEDURE `sp_GetUserInterestGroups`(in p_userSysID int(10) unsigned )
BEGIN
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`hehui`@`200.200.200.17` PROCEDURE `sp_LogUserIn`(in p_userID varchar(16),  in p_hostAddress varchar(64), in p_hostPort smallint unsigned, out p_loginTime datetime)
BEGIN
    set p_loginTime = now();
    insert into LoginHistories(UserID, ExtIPAddress, ExtPort, LoginTime) values(p_userID, p_hostAddress, p_hostPort, p_loginTime); 
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
CREATE DEFINER=`hehui`@`200.200.200.17` PROCEDURE `sp_LogUserOut`(in p_userID varchar(16),  in p_loginTime datetime)
BEGIN
    update LoginHistories set LogoutTime = now() where UserID = p_userID and LoginTime = p_loginTime;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2013-08-05 20:45:52
